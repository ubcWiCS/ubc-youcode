...
const FaqItem = ({ question, answer, isOpen, onClick }) => {
  ...
  return (
    <div className="mb-[6px] md:mb-4 flex flex-col">
      <button
        onClick={onClick}
        className="text-xs md:text-xs lg:text-lg flex justify-between items-center w-full px-4 py-[2px] md:px-6  md:py-[5px] lg:py-3 text-left text-white md:font-semibold bg-teal/[0.7] rounded-2xl hover:bg-teal focus:outline-none focus:ring-2 focus:ring-teal"
      >
        {question}
        <span>{isOpen ? <MdExpandLess /> : <MdExpandMore />}</span> 
      </button>
      {isOpen && (
        <div className="mt-1 lg:mt-2 p-2 md:p-4 text-xs md:text-xs lg:text-lg  rounded-lg text-white">
          {answer}
        </div>
      )}
    </div>
  ...
  const handleItemClick = index => {
    ...
    return (
      <div className="bg-teal-200 p-3 md:p-6 lg:p-14 rounded-3xl lg:bg-white/[0.1] my-10 mx-auto max-w-6xl" id="faq">
        <p className='text-center text-xs md:text-3xl lg:text-5xl font-bold text-white mb-[2px] md:mb-7 lg:mb-20'> 
          FAQ
        </p>
